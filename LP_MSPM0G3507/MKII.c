...
void InitializeBoosterpack(uint32_t ClockFrequency)
{
  ...
  IOMUX->SECCFG.PINCM[BP_S1_INDEX] = (IOMUX_PINCM_PC_CONNECTED | IOMUX_MODE1 | IOMUX_PINCM_INENA_ENABLE | IOMUX_PINCM_PIPU_ENABLE);
  // S2: Peripheral connected, input enabled, internal resistor enabled, IOMUX mode 1
  IOMUX->SECCFG.PINCM[BP_S2_INDEX] = (IOMUX_PINCM_PC_CONNECTED | IOMUX_MODE1 | IOMUX_PINCM_INENA_ENABLE | IOMUX_PINCM_PIPU_ENABLE);
  // Buzzer:
  //IOMUX->SECCFG.PINCM[BUZZER_INDEX] = (IOMUX_PINCM_PC_CONNECTED | IOMUX_MODE4 | )
