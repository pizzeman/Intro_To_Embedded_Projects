...
//UART Protocal: 1 start bit, 8 data bits, no parity, and 1 stop bit
//Write to TXData and read from RXData
void PrintOutputBuffer(UART_Regs *uart, uint8_t *OutputBuffer)
{
    ...
    //Stay in while loop until not full
    while ((uart->STAT & UART_STAT_TXFF_MASK) != 0) {
    }
...
{
    // Wait while RxFIFO is empty
    while ((uart->STAT & UART_STAT_RXFE_MASK) != 0){
    }
